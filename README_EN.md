...
- The nether-star player-info menu's skill tab now opens orelia-core's weapon-skill screen (learn/level-up/socket skills onto the weapon in the player's leftmost hotbar slot, not main hand - right-clicking the Nether Star itself requires holding it in the main hand). The same menu's "実績" (achievement) icon only shows up when OreliaExtra is installed, and opens the achievement GUI directly via `rpg.extra.api.AchievementApi` instead of relaying through a command (the icon is simply omitted when OreliaExtra isn't installed).
