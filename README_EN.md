...
- The nether-star player-info menu's skill tab now opens orelia-core's weapon-skill screen directly via `rpg.api.GuiApi#openSkill`, with no intermediate screen (it used to route through `PlayerInfoSkillGuiScreen`, a "your learned skills" list screen, so skill was a 2-hop navigation while status/achievement were 1-hop). The screen targets whichever weapon sits in any of the player's leftmost 3 hotbar slots, not main hand (right-clicking the Nether Star itself requires holding it in the main hand). The same menu's "実績" (achievement) icon only shows up when OreliaExtra is installed, and opens the achievement GUI directly via `rpg.extra.api.AchievementApi` instead of relaying through a command (the icon is simply omitted when OreliaExtra isn't installed).
- Quests — prerequisite quests (`quests.yml`'s `prerequisite-quests`) already worked and now also notify the player when completing one unlocks another. Repeatable quests (`repeatable: true`) can set `cooldown-hours` to require a wait before re-accepting (unset/0 = instantly re-acceptable, same as before). `/ol quest list` now shows a progress bar per objective. A `party-only: true` quest can't be accepted without a party (always ineligible if OreliaExtra isn't installed). The quest GUI lets you shift-click an in-progress/awaiting-report quest to abandon it, same as `/ol quest abandon`. Finishing every objective now shows a title + action-bar cue instead of a chat line (toggle with `config.yml`'s `quest.completion-feedback.enabled`).
