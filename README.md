...
- ネザースターのプレイヤー情報メニュー(スキルタブ)から orelia-core の武器スキル画面(習得・レベルアップ・武器へのスキル装着)を開けます。この画面はメインハンドではなくホットバーの一番左のスロットに入っている武器を対象にします(ネザースターを右クリックするにはそれをメインハンドに持つ必要があるため)。同メニューの「実績」アイコンは OreliaExtra 導入時のみ表示され、`rpg.extra.api.AchievementApi` 経由でコマンドを介さず実績GUIを直接開きます(未導入時はアイコン自体が表示されません)。
