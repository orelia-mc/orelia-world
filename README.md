...
- `type: RELIC_UPGRADE` のNPC(話しかけるとorelia-coreの「選べる厳選」画面がそのまま開きます、`rpg.api.RelicApi#openUpgradeGui`)、ショップNPCの `shop-stock` に `kind: RELIC`(idはorelia-coreの `relics.yml` `shop-relics:` のキー)が使えます。同梱のデフォルト `npc.yml` では装飾品商人(`accessory_merchant`)の在庫を固定アクセサリーからレリック(`shop_guardian_charm`等)に差し替え、新たに厳選師NPC(`relic_upgrader`)を追加していますが、これはあくまで新規サーバー向けの初期テンプレートです。既にNPCを配置済みの稼働中サーバーには自動反映されません(`ConfigMigrator`は既存キーの追記のみ行い、既存の値は上書きしません)。既存サーバーで同様に切り替える場合は、対象NPCの `npc.yml` エントリを手動編集するか、`/oladmin npc create relic_upgrader RELIC_UPGRADE` で新規NPCとして追加してください。
- ネザースターのプレイヤー情報メニュー(スキルタブ)から orelia-core の武器スキル画面(習得・レベルアップ・武器へのスキル装着)を、中間画面を経由せず`rpg.api.GuiApi#openSkill`で直接開きます(以前は`PlayerInfoSkillGuiScreen`という習得済みスキル一覧の中間画面を挟んでおり、ステータス/実績ボタンだけ1クリックで画面遷移するのに対しスキルだけ2段階になっていました)。この画面はメインハンドではなくホットバーの左端3スロットのいずれかに入っている武器を対象にします(ネザースターを右クリックするにはそれをメインハンドに持つ必要があるため)。同メニューの「実績」アイコンは OreliaExtra 導入時のみ表示され、`rpg.extra.api.AchievementApi` 経由でコマンドを介さず実績GUIを直接開きます(未導入時はアイコン自体が表示されません)。
- クエスト — 前提クエスト(`quests.yml`の`prerequisite-quests`)は既存の仕組みで動作し、解放されると通知が届きます(クエスト名部分はクリックで`/ol quest gui`を実行できます)。リピート可能クエスト(`repeatable: true`)は`cooldown-hours`で再受注までの待機時間を設定できます(未設定/0は従来通り即再受注可)。`/ol quest list`には各クエストの目標ごとに進行バーが表示されます。`party-only: true`のクエストはパーティー未所属だと受注できません(OreliaExtra未導入時は常に受注不可)。`/ol quest gui`でNPCと会話せずともクエストログGUI(`QuestGuiScreen`)を直接開けます — クエスト種別(MAIN/SUB/DAILY/WEEKLY/EVENT)ごとにカテゴリ分けされ、7件を超える一覧はページ送り(`GuiPaginator`)になります。現在受注できないクエストも一覧には表示され、ロック理由(レベル不足・前提未達成など)がlore表示されます。クエストGUIでは進行中/報告待ちのクエストをShift+クリックすると`/ol quest abandon`と同様に放棄できます。目標を全て達成した瞬間はチャットではなくタイトル+アクションバーで通知されます(`config.yml`の`quest.completion-feedback.enabled`で無効化可)。デフォルトで23個のクエスト(MAIN/SUB/DAILY/WEEKLY/EVENT、Lv1〜40)を同梱しており、`start-dialogue-id`/`complete-dialogue-id`(`quests.yml`、任意)でクエスト受注/報告時に`dialogues.yml`のダイアログツリーを再生できます(主要MAINクエスト数個のみ設定済み、NPC演出の一例)。
